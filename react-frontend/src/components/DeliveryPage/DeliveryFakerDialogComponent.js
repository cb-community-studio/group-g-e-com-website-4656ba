...
import deliveryFakerFactory from "./deliveryFakerFactory";
...
const DeliveryFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deliveryFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("delivery").create(elem));
...
export default connect(null, mapDispatch)(DeliveryFakerDialogComponent);
