...
import seederData from "./shippingorder.seeder.json";
...
const ShippingorderSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("shippingorder").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ShippingorderSeederDialogComponent);
