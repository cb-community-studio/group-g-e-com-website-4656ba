...
import shippingorderFakerFactory from "./shippingorderFakerFactory";
...
const ShippingorderFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = shippingorderFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("shippingorder").create(elem));
...
export default connect(null, mapDispatch)(ShippingorderFakerDialogComponent);
